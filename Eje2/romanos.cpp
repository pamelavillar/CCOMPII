...
#include <vector>
using namespace std;
// Apartir de 3999, se añade una linea encima del numero romano millar,
// indicando que el numero se multiplica por 1000 3999 -> MMMCMXCIX
...
int main() {
  ...
  while (re) {
    ...
    while (nu > 0) {
      if (s >= 4 && nu >= 4) { // este caso es porque apartir del 3999, el
                               // millar se multiplica,
        // entonces es como hacer borron y cuenta nueva xd, esta arriba
        // explicado :b
        s = 1;
        romano.push_back("-");
        try0(nu % 10, s, romano);
